    ...
    >
      <div class="off_canvas_body">
        ...
        <div class="data_content">
          ...
          <div class="filter_item">
            <button
              @click.prevent="date_wise_data(param_date)"
              type="button"
...
export default {
  ...
  created: async function () {
    this.param_date = this.$route.params.id;
    await this.date_wise_data(this.param_date);
    // await this.get_all();
  },
  ...
  methods: {
    ...
    change_status: async function (status = "active") {
      if (status == "trased") {
      ...
      this.set_only_latest_data(true);
      // ensure status is set before fetching
      await this.set_status(status);
      this.set_page(1);
      // fetch filtered data for the current date
      await this.date_wise_data(this.param_date);
      this.set_only_latest_data(true);
    },
    ...
    set_page_data: function (link) {
      try {
        ...
        // this.get_all();
        this.date_wise_data(this.param_date);
      } catch (error) {}
    },
