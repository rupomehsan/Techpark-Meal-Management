        ...
        <div class="card-header d-flex justify-content-between">
          ...
          <div>
            ...
            <router-link
              ...
              :to="{ name: `Expense${setup.route_prefix}` }"
            >
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h1 class="modal-title fs-5" id="staticBackdropLabel">Add Cook Salary</h1>
              </div>
              <div class="modal-body">
                <form @submit.prevent="submitAddCookSalary">
                  <div class="mb-3">
                    <label for="date" class="form-label">Date</label>
                    <input type="date" name="salary_date" class="form-control" id="date" required/>
                  </div>
                  <div class="mb-3">
                    <label for="salary" class="form-label">Cook Salary</label>
                    <input type="number" name="cook_salary" class="form-control" id="salary" required/>
                  </div>
...
export default {
  data: () => ({
    ...
    cook_salary: '',
      rows: [
        { id: 1, title: '', quantity: '', cook_salary: '', unit: '', price: '', total: '' }
      ],
      nextId: 2,
  ...
  methods: {
    ...
    submitAddCookSalary: async function (event) {
      try {
        const formData = new FormData(event.target);
        const response = await axios.post('daily-bajars/cook-salary/store', formData);
        this.item = response.data.data;
        window.s_alert("Cook Salary Successfully Created");
